 ...
 */
export function getWaveFn(fn: (radians: number) => number, p = 1000, min = -1, max = 1, o = 0) {
 ...
 */
export function getStepFn(fn: (radians: number) => number, p = 1000, min = -1, max = 1, o = 0) {
  ...
  return (t: number) => Math.floor(waveFn(t));
}
